...
am_tmux_config_path() {
    ...
    local am_cmd switch_cmd kill_cmd cycle_cmd index_cmd status_bar_cmd toggle_cmd desired_conf
...
# The agent branch stays fork-free (hot path); the shell branch is
# layout-aware: it fails when the session has no shell panel (a lone pane
# makes tmux's {bottom} alias to the agent pane, which callers must never
# silently target) and follows the panel into the hidden window when parked.
...
# below the agent), or hidden (parked in the $AM_SHELL_WINDOW window).
# Usage: tmux_shell_pane_state <session_name>
tmux_shell_pane_state() {
    local session="$1"
    local window main_panes=0 has_hidden=false
    ...
    done < <(am_tmux list-panes -s -t "$session" -F '#{window_name}' 2>/dev/null)
...
# Window id (@N) of the session's main window — the one that is not the
# hidden shell-parking window. Window ids are unambiguous even when the
# session's current window is the hidden one.
# Usage: tmux_main_window_id <session_name>
tmux_main_window_id() {
    local session="$1"
    am_tmux list-windows -t "$session" -F '#{window_id} #{window_name}' 2>/dev/null \
        | awk -v skip="$AM_SHELL_WINDOW" '$2 != skip {print $1; exit}'
}
...
tmux_shell_pane_hide() {
    ...
    am_tmux break-pane -d -n "$AM_SHELL_WINDOW" -s "${main_id}.{bottom}" || return 1
    # Lone agent pane: drop the border-status line so no row is wasted.
...
tmux_shell_pane_show() {
    ...
    am_tmux join-pane -v -l 15 -s "${session}:${AM_SHELL_WINDOW}" -t "${main_id}.{top}" || return 1
    ...
    am_tmux select-pane -t "${main_id}.{bottom}" 2>/dev/null || true
}
