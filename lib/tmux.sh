# shellcheck shell=bash
# tmux.sh - tmux wrapper functions

# Source utils if not already loaded
[[ -z "$AM_DIR" ]] && source "${AM_LIB_DIR:-$(dirname "${BASH_SOURCE[0]}")}/utils.sh"

# Window name used to park a hidden shell panel (break-pane target). Pane
# enumeration that assumes "one window per session" must skip this window.
AM_SHELL_WINDOW="_amshell"
# Hidden window the collapsible review pane is parked in (lib/review.sh),
# and the pane's width (tmux -l: columns or a percentage).
AM_REVIEW_WINDOW="_amreview"
AM_REVIEW_WIDTH="${AM_REVIEW_WIDTH:-45%}"

am_tmux_config_path() {
    # Cache: only generate config once per process
    if [[ -n "${_AM_TMUX_CONF_CACHED:-}" ]]; then
        echo "$_AM_TMUX_CONF_CACHED"
        return
    fi

    am_init

    local am_cmd switch_cmd kill_cmd cycle_cmd index_cmd status_bar_cmd toggle_cmd review_cmd desired_conf
    am_cmd="$AM_ROOT_DIR/am"
    switch_cmd="$AM_ROOT_DIR/bin/switch-last"
    kill_cmd="$AM_ROOT_DIR/bin/kill-and-switch"
    cycle_cmd="$AM_ROOT_DIR/bin/switch-cycle"
    index_cmd="$AM_ROOT_DIR/bin/switch-index"
    status_bar_cmd="$AM_ROOT_DIR/lib/status-bar"
    toggle_cmd="$AM_ROOT_DIR/bin/toggle-shell"
    review_cmd="$AM_ROOT_DIR/bin/toggle-review"

    desired_conf=$(cat <<EOF
# Generated by agent-manager. Edit ~/.tmux.conf for your base tmux settings.
# This config runs on a dedicated tmux server (socket: $AM_TMUX_SOCKET), so
# every session here is an am-* session — bindings don't need an am-* guard.
# Prefix+[ keeps tmux's default copy-mode; prefix+] keeps default paste-buffer.
if-shell '[ -f "$HOME/.tmux.conf" ]' 'source-file "$HOME/.tmux.conf"'
set -g detach-on-destroy off
set -g focus-events on

# Extended keys: let Claude in the agent pane tell Shift+Enter (insert newline)
# apart from Enter (submit), plus other modified keys. 'on' (not 'always') only
# sends the CSI-u encoding to apps that request the mode, so shell-pane tools are
# untouched. The terminal-features line tells tmux the outer terminal can carry
# it (iTerm2/Ghostty/kitty/WezTerm do; Terminal.app doesn't — harmless there).
set -g extended-keys on
set -as terminal-features 'xterm*:extkeys'
# Emit the CSI-u encoding (not the older xterm/modifyOtherKeys form). Agents
# like Claude/Pi parse csi-u more reliably; server option, so use -s.
set -s extended-keys-format csi-u

# OSC 8 hyperlinks: tmux only forwards them when the client advertises the
# 'hyperlinks' feature, which it reads from the terminfo Hls capability. No
# entry sets Hls -- not xterm-256color, not Ghostty's own xterm-ghostty -- so
# force it, or clickable links from agents arrive as plain text. Terminals
# without OSC 8 support ignore the sequence.
set -as terminal-features '*:hyperlinks'

# Restore tmux defaults for [/] (idempotent on reload — earlier versions of
# this config rebound them to session cycling).
bind [ copy-mode
bind ] paste-buffer -p

# Prefix + a: switch to last used am session
bind a run-shell "$switch_cmd"

# Prefix + Right/Left: cycle next/prev session in canonical (creation-time) order
bind Right run-shell "$cycle_cmd next"
bind Left run-shell "$cycle_cmd prev"

# Prefix + 1-9: jump to sidebar slot N
bind 1 run-shell "$index_cmd 1"
bind 2 run-shell "$index_cmd 2"
bind 3 run-shell "$index_cmd 3"
bind 4 run-shell "$index_cmd 4"
bind 5 run-shell "$index_cmd 5"
bind 6 run-shell "$index_cmd 6"
bind 7 run-shell "$index_cmd 7"
bind 8 run-shell "$index_cmd 8"
bind 9 run-shell "$index_cmd 9"

# Prefix + n: open new-session popup (tmux clamps to the client size)
bind n display-popup -E -w 160 -h 32 "$am_cmd new"

# Prefix + s: open agent manager popup
bind s display-popup -E -w 90% -h 80% "$am_cmd"

# Prefix + h: open the merged session switcher, which includes inactive sessions
bind h display-popup -E -w 90% -h 80% "$am_cmd"

# Prefix + x: kill the current am session and switch to the next most recent one
bind-key -T prefix x run-shell "$kill_cmd #{client_name} #{session_name}"

# Prefix + \`: toggle the shell panel below the agent (VS Code-style).
# First use creates it; after that it hides/shows the same pane, so cwd,
# history, and running jobs survive.
bind-key -T prefix '\`' run-shell "$toggle_cmd #{session_name}"

# Prefix + v: toggle the review pane at the agent's right — what the agent
# changed since you last looked (am diff), refreshing on every tool event.
bind-key -T prefix v run-shell "$review_cmd #{session_name}"

# Optional alias: :am
set -s command-alias[100] am='display-popup -E -w 90% -h 80% "$am_cmd"'

# Status bar: whole bottom bar acts as a tab switcher. The status-bar script
# precomputes per-session @am_status_left / @am_sidebar options, and tmux
# just reads those — switching sessions is instant because no #() job needs
# to finish first. A hidden invocation in status-right triggers periodic
# refresh at status-interval. The right side names the current session (its
# am id) and the clock; status-bar reserves exactly " <id> HH:MM " for it, so
# keep the two in step.
set -g status-interval 5
set -g status-style 'bg=colour234,fg=colour250'
set -g status-left-length 500
set -g status-left '#{@am_status_left}'
set -g status-right-length 120
set -g status-right '#($status_bar_cmd #{session_name}) #[fg=colour245]#{session_name} %H:%M '

# Refresh options after the active session changes. run-shell -b detaches
# the child so tmux doesn't block on it — switch-client returns immediately
# and the user is in the new session before status-bar finishes. The
# precomputed @am_status_left option already has the right "current"
# highlighting baked in (status-bar writes one per perspective), so the
# visual update is instant; this hook just keeps state glyphs and ages
# fresh between the 5s status-interval ticks.
set-hook -g client-session-changed 'run-shell -b "$status_bar_cmd #{session_name}"'

# Drop tmux's default window list — am sessions use a single window so the
# default 'index:name*' slot is just clutter between tabs and the clock.
set -g window-status-format ''
set -g window-status-current-format ''
set -g window-status-separator ''
set -g status-justify left

# Pane border sidebar: compact session list on the divider between agent and
# shell panes. Reads the @am_sidebar option populated by status-bar.
# Sessions without an open shell panel override pane-border-status to off at
# the window level (see agent_launch / toggle-shell) so the lone agent pane
# keeps the full height.
set -g pane-border-status bottom
set -g pane-border-format '#{?#{pane_at_bottom},,#[align=centre]#{@am_sidebar}}'
set -g pane-border-lines double
set -g pane-border-style 'fg=colour67'
set -g pane-active-border-style 'fg=colour14'

# Clicking a session tab in the status bar switches to that session. Tabs are
# wrapped in #[range=user|<session>]...#[norange] by lib/status-bar, so
# mouse_status_range carries the session name directly. switch-client -t does
# NOT format-expand its target arg, so we shell out via run-shell (whose
# shell-command arg IS format-expanded) instead.
# Border-level click isn't possible — tmux doesn't populate mouse_x/mouse_y
# for MouseDown1Border events.
set -g mouse on

# Double-click word selection tuned like iTerm2's native selection: path and
# identifier chars ($ \` + - . / \\ _ ~ plus alnum) stay joined so /path/to/file,
# --flag, my_var-name.ext and $HOME select as one unit, while operators and
# brackets ( = : , @ | [ ] ( ) etc.) split — so key=value and host:port break
# apart. tmux's default treats every punctuation mark as a boundary (too choppy);
# a lone space was too loose (never split key=value).
set -g word-separators " !\"#%&'()*,:;<=>?@[]^{|}"
bind -n MouseDown1Status if-shell -F '#{!=:#{mouse_status_range},}' { run-shell "tmux switch-client -t '#{mouse_status_range}' ; tmux refresh-client -S" } { switch-client -t = }
EOF
)

    # Refresh generated config when the repo path or binding format changes.
    if [[ ! -f "$AM_TMUX_CONF" ]] || [[ "$(cat "$AM_TMUX_CONF" 2>/dev/null)" != "$desired_conf" ]]; then
        printf '%s\n' "$desired_conf" > "$AM_TMUX_CONF"

        # If the tmux server is already running, source the updated config
        # so changes (like status-right) take effect without restarting.
        command tmux -L "$AM_TMUX_SOCKET" source-file "$AM_TMUX_CONF" 2>/dev/null || true

        # Clear legacy per-session status-left overrides left over from the old
        # tmux_set_session_status code path so existing sessions pick up the
        # global tab-strip status-left.
        local _legacy_session
        while IFS= read -r _legacy_session; do
            [[ -n "$_legacy_session" ]] || continue
            command tmux -L "$AM_TMUX_SOCKET" set-option -u -t "$_legacy_session" status-left 2>/dev/null || true
            command tmux -L "$AM_TMUX_SOCKET" set-option -u -t "$_legacy_session" status-left-length 2>/dev/null || true
        done < <(command tmux -L "$AM_TMUX_SOCKET" list-sessions -F '#{session_name}' 2>/dev/null \
                    | grep "^${AM_SESSION_PREFIX}" || true)
        command tmux -L "$AM_TMUX_SOCKET" refresh-client -S 2>/dev/null || true
    fi

    _AM_TMUX_CONF_CACHED="$AM_TMUX_CONF"
    echo "$AM_TMUX_CONF"
}

am_tmux() {
    if [[ -z "${_AM_TMUX_CONF_CACHED:-}" ]]; then
        am_tmux_config_path >/dev/null
    fi
    command tmux -L "$AM_TMUX_SOCKET" -f "$_AM_TMUX_CONF_CACHED" "$@"
}

am_tmux_is_current_server() {
    [[ "${TMUX:-}" == *"/${AM_TMUX_SOCKET},"* ]]
}

# Check if a tmux session exists
# Usage: tmux_session_exists <name>
tmux_session_exists() {
    local name="$1"
    am_tmux has-session -t "$name" 2>/dev/null
}

# Create a new tmux session (detached). Extra VAR=VALUE args seed the initial
# pane's environment (tmux -e, 3.2+) and the session environment, so the first
# shell and every later split in the session inherit them — no typed `export`
# lines, nothing in shell history.
# Usage: tmux_create_session <name> <directory> [VAR=VALUE...]
tmux_create_session() {
    local name="$1"
    local directory="$2"
    shift 2

    if tmux_session_exists "$name"; then
        log_error "Session '$name' already exists"
        return 1
    fi

    local -a env_flags=()
    local kv
    for kv in "$@"; do
        env_flags+=(-e "$kv")
    done

    # Create with explicit dimensions to work around tmux sizing bugs in detached sessions
    # https://github.com/tmux/tmux/issues/3060
    am_tmux new-session -d -s "$name" -c "$directory" -x 200 -y 60 "${env_flags[@]}" || return 1
    for kv in "$@"; do
        am_tmux set-environment -t "$name" "${kv%%=*}" "${kv#*=}"
    done
}

# Kill a tmux session
# Usage: tmux_kill_session <name>
tmux_kill_session() {
    local name="$1"

    if ! tmux_session_exists "$name"; then
        log_warn "Session '$name' does not exist"
        return 1
    fi

    tmux_cleanup_logs "$name"
    am_tmux kill-session -t "$name"
}

# Create am-owned scratch directories under /tmp (pane logs, hook state)
# readable by the owner only. They hold complete agent/shell scrollback and
# conversation ids, so they get the same 0700 as ~/.agent-manager. umask 077
# covers every level `mkdir -p` creates; the chmod tightens a directory an
# earlier release created 0755 (a no-op once it is 0700).
# Usage: am_mkdir_private <dir> [dir...]
am_mkdir_private() {
    local d
    for d in "$@"; do
        [[ -d "$d" ]] || (umask 077; mkdir -p "$d")
        chmod 700 "$d" 2>/dev/null || true
    done
}

# Stream pane output to a log file using tmux pipe-pane
tmux_enable_pipe_pane() {
    local session="$1"
    local pane="$2"
    local log_file="$3"

    tmux_pipe_pane "${session}:${pane}" "$log_file"
}

# Same as tmux_enable_pipe_pane but takes a raw pane target (e.g. a %id).
# The log file is created by the pipe command's own redirect, inside the tmux
# server's environment, so the umask travels with the command: 0600, matching
# the directory am_mkdir_private gives it.
tmux_pipe_pane() {
    local target="$1"
    local log_file="$2"

    am_tmux pipe-pane -t "$target" -o "umask 077; ${AM_LIB_DIR}/strip-ansi >> ${log_file}"
}

# Remove log directory and legacy sidebar cache for a session
tmux_cleanup_logs() {
    local name="$1"
    local log_dir="/tmp/am-logs/${name}"

    if [[ -d "$log_dir" ]]; then
        rm -rf "$log_dir"
    fi
    rm -f "/tmp/am-sidebar/${name}"
}

# Attach to a tmux session
# Usage: tmux_attach <name>
tmux_attach() {
    local name="$1"

    if ! tmux_session_exists "$name"; then
        log_error "Session '$name' does not exist"
        return 1
    fi

    if am_tmux_is_current_server; then
        # Already inside the agent-manager tmux server, switch client in place.
        am_tmux switch-client -t "$name"
    else
        # Attach to the dedicated agent-manager server. Clear TMUX so nesting
        # inside an unrelated tmux server attaches a new client instead.
        TMUX='' am_tmux attach-session -t "$name"
    fi
}

# Get a tmux session field value
# Usage: _tmux_get_session_field <name> <format_token>
_tmux_get_session_field() {
    local name="$1" format="$2"
    am_tmux list-sessions -F "#{session_name} #{$format}" 2>/dev/null \
        | grep "^${name} " | cut -d' ' -f2
}

# Get session activity timestamp (seconds since epoch)
tmux_get_activity() { _tmux_get_session_field "$1" session_activity; }

# Get session creation time (seconds since epoch)
tmux_get_created() { _tmux_get_session_field "$1" session_created; }

# List all agent-manager sessions (those with AM_SESSION_PREFIX)
# Usage: tmux_list_am_sessions
# Returns: newline-separated session names
tmux_list_am_sessions() {
    am_tmux list-sessions -F '#{session_name}' 2>/dev/null \
        | grep "^${AM_SESSION_PREFIX}" \
        || true
}

# List all agent-manager sessions in canonical sidebar order: creation time
# ascending, so newly created sessions always appear at the end. Stable — only
# changes on session create/kill. Numbered slots and cycling keybindings
# follow this order.
# Usage: am_session_order
# Returns: newline-separated session names
am_session_order() {
    am_tmux list-sessions -F '#{session_created} #{session_name}' 2>/dev/null \
        | grep " ${AM_SESSION_PREFIX}" \
        | sort -k1,1n -k2,2 \
        | awk '{print $2}' \
        || true
}

# Regenerate per-session status-bar options (@am_status_left, @am_sidebar) for
# every am session and force a client-wide redraw. Call after creating or
# killing a session so the status bar and pane border update immediately
# instead of waiting for the 5s status-interval refresh.
#
# A single status-bar invocation already writes options for every session in
# its final loop, so this just runs it once — the previous per-session loop
# performed the exact same work N times.
# Usage: am_refresh_sidebar_cache
am_refresh_sidebar_cache() {
    local script="${AM_LIB_DIR:-$(dirname "${BASH_SOURCE[0]}")}/status-bar"
    [[ -x "$script" ]] || return 0
    "$script" >/dev/null 2>&1 || true

    # Clean up stale files from the old pane-border implementation. That path
    # used tmux #() jobs, which are cached and caused delayed middle-bar updates.
    local cache_dir="/tmp/am-sidebar" cache_file
    if [[ -d "$cache_dir" ]]; then
        for cache_file in "$cache_dir"/"${AM_SESSION_PREFIX}"*; do
            [[ -e "$cache_file" ]] || continue
            rm -f "$cache_file"
        done
        rmdir "$cache_dir" 2>/dev/null || true
    fi

    am_tmux refresh-client -S 2>/dev/null || true
}

# Send keys to a tmux session
# Usage: tmux_send_keys <name> <keys...>
tmux_send_keys() {
    local name="$1"
    shift
    am_tmux send-keys -t "$name" "$@"
}

# Paste literal text into a tmux pane without shell escaping issues.
# -p wraps the text in bracketed-paste markers (ESC[200~ … ESC[201~) when the
# pane's application has enabled bracketed paste, as every agent TUI does —
# the same flag the prefix+] binding uses. Without it tmux converts each LF
# to CR, so a multi-line prompt reaches Claude as one submission per line
# instead of one block that the caller then submits with a single Enter.
# Usage: tmux_paste_text <target-pane> <text>
tmux_paste_text() {
    local target="$1"
    local text="$2"

    printf '%s' "$text" | am_tmux load-buffer -
    am_tmux paste-buffer -d -p -t "$target"
}

# Return the title of a pane (set by the application via escape sequences).
# Usage: tmux_pane_title <target-pane>
tmux_pane_title() {
    local target="$1"
    am_tmux display-message -p -t "$target" '#{pane_title}'
}

# Pane roles. am panes are tagged with the pane option @am_role (shell,
# review) when they are created; the agent pane is the untagged one. Roles
# travel with the pane through break-pane/join-pane, so hide/show and target
# resolution never depend on the layout: with a review pane at the agent's
# right and a shell below, tmux's {bottom} could be either of them, and
# {top} either the agent or the review pane. The agent branch keeps {top}
# only because the agent pane is pane 0 of the window and tmux breaks the
# tie by index. Sessions from before role tagging have an untagged shell
# pane; the shell helpers fall back to {bottom} for them (no review pane can
# exist there — it is created tagged).
# Usage: tmux_pane_role_set <pane_id> <role>
tmux_pane_role_set() {
    am_tmux set-option -p -t "$1" @am_role "$2"
}

# Pane id (%N) of the main-window pane with the given role, or empty.
# Usage: tmux_session_pane_by_role <session_name> <role>
tmux_session_pane_by_role() {
    local session="$1" role="$2" pane_id window pane_role
    while read -r pane_id window pane_role; do
        [[ "$window" == "$AM_SHELL_WINDOW" || "$window" == "$AM_REVIEW_WINDOW" ]] && continue
        if [[ "$pane_role" == "$role" ]]; then
            echo "$pane_id"
            return 0
        fi
    done < <(am_tmux list-panes -s -t "$session" -F '#{pane_id} #{window_name} #{@am_role}' 2>/dev/null)
    return 1
}

# Resolve a logical pane name to a tmux pane target.
# The agent branch stays fork-free (hot path); the shell and review branches
# are layout-aware: they fail when the session has no such pane (a lone pane
# makes tmux's {bottom} alias to the agent pane, which callers must never
# silently target) and follow the pane into its hidden window when parked.
# Usage: tmux_session_pane_target <session_name> [agent|shell|review]
tmux_session_pane_target() {
    local session_name="$1"
    local pane_name="${2:-agent}"

    case "$pane_name" in
        agent|top) echo "${session_name}:.{top-left}" ;;
        shell|bottom)
            case "$(tmux_shell_pane_state "$session_name")" in
                open) tmux_shell_pane_id "$session_name" ;;
                hidden) echo "${session_name}:${AM_SHELL_WINDOW}" ;;
                *) return 1 ;;
            esac
            ;;
        review)
            case "$(tmux_review_pane_state "$session_name")" in
                open) tmux_session_pane_by_role "$session_name" review ;;
                hidden) echo "${session_name}:${AM_REVIEW_WINDOW}" ;;
                *) return 1 ;;
            esac
            ;;
        *) return 1 ;;
    esac
}

# Target of the open shell pane: its tagged pane id, else (pre-tagging
# session) the window's bottom pane.
# Usage: tmux_shell_pane_id <session_name>
tmux_shell_pane_id() {
    local session="$1" pane_id
    if pane_id=$(tmux_session_pane_by_role "$session" shell); then
        echo "$pane_id"
    else
        echo "${session}:.{bottom-left}"
    fi
}

# Shell panel state for a session: absent (never opened), open (visible
# below the agent), or hidden (parked in the $AM_SHELL_WINDOW window). Panes
# tagged review do not count: agent + review is still "no shell panel".
# Usage: tmux_shell_pane_state <session_name>
tmux_shell_pane_state() {
    local session="$1"
    local window role main_panes=0 has_hidden=false
    while read -r window role; do
        if [[ "$window" == "$AM_SHELL_WINDOW" ]]; then
            has_hidden=true
        elif [[ "$window" == "$AM_REVIEW_WINDOW" || "$role" == "review" ]]; then
            continue
        else
            main_panes=$((main_panes + 1))
        fi
    done < <(am_tmux list-panes -s -t "$session" -F '#{window_name} #{@am_role}' 2>/dev/null)

    if $has_hidden; then
        echo "hidden"
    elif (( main_panes >= 2 )); then
        echo "open"
    else
        echo "absent"
    fi
}

# Review pane state: absent (not open), open (beside the agent), or hidden
# (parked in the $AM_REVIEW_WINDOW window). Purely tag/window based.
# Usage: tmux_review_pane_state <session_name>
tmux_review_pane_state() {
    local session="$1"
    local window role has_open=false has_hidden=false
    while read -r window role; do
        if [[ "$window" == "$AM_REVIEW_WINDOW" ]]; then
            has_hidden=true
        elif [[ "$role" == "review" ]]; then
            has_open=true
        fi
    done < <(am_tmux list-panes -s -t "$session" -F '#{window_name} #{@am_role}' 2>/dev/null)

    if $has_hidden; then
        echo "hidden"
    elif $has_open; then
        echo "open"
    else
        echo "absent"
    fi
}

# Window id (@N) of the session's main window — the one that is not a
# hidden parking window. Window ids are unambiguous even when the session's
# current window is a hidden one.
# Usage: tmux_main_window_id <session_name>
tmux_main_window_id() {
    local session="$1"
    am_tmux list-windows -t "$session" -F '#{window_id} #{window_name}' 2>/dev/null \
        | awk -v skip1="$AM_SHELL_WINDOW" -v skip2="$AM_REVIEW_WINDOW" '$2 != skip1 && $2 != skip2 {print $1; exit}'
}

# pane-border-status for the main window: off when the agent is alone (no
# row wasted), tmux.conf's default otherwise.
# Usage: _tmux_border_status_sync <window_id>
_tmux_border_status_on() {
    am_tmux set-option -w -u -t "$1" pane-border-status
}

_tmux_border_status_sync() {
    local main_id="$1" count
    count=$(am_tmux list-panes -t "$main_id" 2>/dev/null | wc -l | tr -d ' ')
    if (( count <= 1 )); then
        am_tmux set-option -w -t "$main_id" pane-border-status off
    else
        am_tmux set-option -w -u -t "$main_id" pane-border-status
    fi
}

# Park the open shell panel in the hidden window. The pane keeps running
# (cwd, history, jobs, pipe-pane streaming all survive).
# Usage: tmux_shell_pane_hide <session_name>
tmux_shell_pane_hide() {
    local session="$1"
    local main_id source
    main_id=$(tmux_main_window_id "$session")
    [[ -n "$main_id" ]] || return 1
    source=$(tmux_shell_pane_id "$session")
    [[ "$source" == %* ]] || source="${main_id}.{bottom-left}"

    am_tmux break-pane -d -n "$AM_SHELL_WINDOW" -s "$source" || return 1
    _tmux_border_status_sync "$main_id"
    am_tmux set-option -w -t "${session}:${AM_SHELL_WINDOW}" pane-border-status off
}

# Rejoin the parked shell panel below the agent pane and focus it.
# Usage: tmux_shell_pane_show <session_name>
tmux_shell_pane_show() {
    local session="$1"
    local main_id
    main_id=$(tmux_main_window_id "$session")
    [[ -n "$main_id" ]] || return 1

    am_tmux join-pane -v -l 15 -s "${session}:${AM_SHELL_WINDOW}" -t "${main_id}.{top-left}" || return 1
    _tmux_border_status_sync "$main_id"
    am_tmux select-window -t "$main_id" 2>/dev/null || true
    am_tmux select-pane -t "$(tmux_shell_pane_id "$session")" 2>/dev/null || true
}

# Park the open review pane in its hidden window (the TUI keeps running, so
# its scroll position and selection survive).
# Usage: tmux_review_pane_hide <session_name>
tmux_review_pane_hide() {
    local session="$1"
    local main_id source
    main_id=$(tmux_main_window_id "$session")
    [[ -n "$main_id" ]] || return 1
    source=$(tmux_session_pane_by_role "$session" review) || return 1

    am_tmux break-pane -d -n "$AM_REVIEW_WINDOW" -s "$source" || return 1
    _tmux_border_status_sync "$main_id"
    am_tmux set-option -w -t "${session}:${AM_REVIEW_WINDOW}" pane-border-status off
    am_tmux select-pane -t "${main_id}.{top-left}" 2>/dev/null || true
}

# Rejoin the parked review pane at the agent's right and focus it.
# Usage: tmux_review_pane_show <session_name>
tmux_review_pane_show() {
    local session="$1"
    local main_id
    main_id=$(tmux_main_window_id "$session")
    [[ -n "$main_id" ]] || return 1

    # Border row first, so the rejoined TUI is resized once, to its final size.
    _tmux_border_status_on "$main_id"
    am_tmux join-pane -h -l "$AM_REVIEW_WIDTH" -s "${session}:${AM_REVIEW_WINDOW}" -t "${main_id}.{top-left}" || return 1
    _tmux_border_status_sync "$main_id"
    am_tmux select-window -t "$main_id" 2>/dev/null || true
    am_tmux select-pane -t "$(tmux_session_pane_by_role "$session" review)" 2>/dev/null || true
}

# Return the optional streamed log file path for a session pane.
# Usage: tmux_session_pane_log_file <session_name> [agent|shell]
tmux_session_pane_log_file() {
    local session_name="$1"
    local pane_name="${2:-agent}"

    case "$pane_name" in
        agent|top) echo "/tmp/am-logs/${session_name}/agent.log" ;;
        shell|bottom) echo "/tmp/am-logs/${session_name}/shell.log" ;;
        *) return 1 ;;
    esac
}

# Capture the visible history of a pane.
# Usage: tmux_capture_pane <target-pane> [lines]
# If lines is specified, captures only the last N lines using -S -N flag.
# Otherwise captures entire history with -S - -E -.
tmux_capture_pane() {
    local target="$1"
    local lines="${2:-}"

    if [[ -n "$lines" && "$lines" =~ ^[0-9]+$ ]]; then
        am_tmux capture-pane -t "$target" -p -e -S "-${lines}" -E - 2>/dev/null
    else
        am_tmux capture-pane -t "$target" -p -e -S - -E - 2>/dev/null
    fi
}

# Count agent-manager sessions
# Usage: tmux_count_am_sessions
tmux_count_am_sessions() {
    local sessions
    sessions=$(tmux_list_am_sessions)
    if [[ -z "$sessions" ]]; then
        echo 0
    else
        echo "$sessions" | wc -l | tr -d ' '
    fi
}
