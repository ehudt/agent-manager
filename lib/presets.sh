...
#
# A preset is a saved set of `am new` inputs: directory, agent type, task,
# workspace allocation (+ branch), shell panel, and extra agent args. It lives
# under the "presets" key of config.json:
#
#   "presets": {
#     "review": {"agent": "claude", "workspace": true, "branch": "",
#                "task": "", "directory": "", "shell": false,
...
# One scalar field of a preset (directory|agent|task|branch as strings,
# workspace|shell as true/false, args newline-separated). Empty when unset.
...
am_preset_field() {
    ...
    case "$field" in
        args)  jq -r '.args // [] | .[]' <<< "$obj" ;;
        workspace|shell) jq -r --arg f "$field" '.[$f] // false' <<< "$obj" ;;
...
# Usage: _preset_from_flags [-t agent] [-d dir] [-n task] [-W [branch]] [--shell|--no-shell] [-- agent args...]
_preset_from_flags() {
    local agent="" directory="" task="" workspace=false branch="" shell=false
    local -a args=()
    while [[ $# -gt 0 ]]; do
        case "$1" in
    ...
    jq -cn --arg agent "$agent" --arg directory "$directory" --arg task "$task" \
        --argjson workspace "$workspace" --arg branch "$branch" --argjson shell "$shell" \
        --args '{agent: $agent, directory: $directory, task: $task, workspace: $workspace,
                 branch: $branch, shell: $shell, args: $ARGS.positional}
...
_preset_render() {
    ...
    local obj
    obj=$(am_preset_get "$name") || return 1
    local agent workspace branch shell task directory
    # \x1f separator: tabs are IFS whitespace and would collapse empty fields
    IFS=$'\x1f' read -r agent workspace branch shell task directory < <(
        jq -r '[.agent // "", (.workspace // false | tostring), .branch // "",
                (.shell // false | tostring), .task // "", .directory // ""] | join("")' <<< "$obj")
    local -a parts=()
    [[ -n "$agent" ]] && parts+=("-t" "$agent")
    if [[ "$workspace" == "true" ]]; then
        parts+=("-W"); [[ -n "$branch" ]] && parts+=("$branch")
    fi
...
preset_main() {
    ...
    case "$sub" in
        save|add)
            local name="${1:-}"
            [[ -n "$name" ]] || { echo "Usage: am preset save <name> [-t agent] [-d dir] [-n task] [-W [branch]] [--shell] [-- agent-args...]" >&2; return 1; }
            shift
            local obj
...
Usage: am preset <save|list|show|rm> ...
  ...
  am preset save <name> [-t agent] [-d dir] [-n task] [-W [branch]] [--shell] [-- agent-args...]
  am preset list
  am preset show <name>
  am preset rm <name>
...
Examples:
  am preset save review -W -- --model opus --effort high
  am preset save scratch -t pi ~/code/tools
  am new -p review pr-48351
EOF
            ;;
        *)
