...
- **Orphan registry rows** — `sessions.json` keeps entries whose tmux session is gone (observed: 3 rows on `agent-manager` socket pointing to nonexistent sessions). `registry_gc` runs on the fzf-fallback path but not on the Go TUI path, so rows accumulate. Reap from `LoadEntries` (Go) or on session-kill hook. Surfaced while reproducing task-detection bug; separate fix.
